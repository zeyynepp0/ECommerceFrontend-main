...
const ProductDetailsPage = ({ darkMode }) => {
  ...
  if (product && product.isActive === false) {
    return (
      <div className={`product-details-page ${darkMode ? 'dark' : ''}`}>
        <div className="product-details-container">
          <div className="product-not-found">
            <h2>Ürün Aktif Değil</h2>
            <p>Bu ürün şu anda satışta değildir.</p>
            <button onClick={() => navigate('/products')} className="back-to-products">
              Tüm Ürünlere Dön
            </button>
          </div>
  ...
  return (
    <CContainer fluid className={`py-4 product-details-page${darkMode ? ' dark' : ''}`}>
      <CRow>
        ...
        <CCol md={6}>
          <CCard className="h-100 p-4">
            ...
            <div className="mb-2 text-muted">Kategori: {product.category?.name || 'Kategori Yok'}</div>
            ...
            <div className="mb-2">
              <CBadge color={stock === 0 ? 'secondary' : 'success'}>{stock === 0 ? 'Stokta Yok' : 'Stokta'}</CBadge>
              {product.discount > 0 && (
                <CBadge color="danger" className="ms-2">%{product.discount} İndirim</CBadge>
              )}
            </div>
            ...
            <div className="d-flex align-items-center gap-3 mb-3">
              ...
              <CButton color="primary" onClick={handleAddToCart} disabled={stock === 0 || (cartQuantity + quantity > stock)}>
                <FiShoppingCart className="me-2" />{stock === 0 ? 'Stokta Yok' : 'Sepete Ekle'}
              </CButton>
              <CButton color={isFavorited ? 'danger' : 'light'} variant={isFavorited ? '' : 'outline'} onClick={handleFavoriteClick} disabled={isFavoriteLoading}>
                <FiHeart className="me-1" />{isFavorited ? 'Favoride' : 'Favoriye Ekle'}
              </CButton>
            </div>
          </CCard>
        </CCol>
      ...
      <CRow className="mt-4">
        <CCol md={8}>
          <CCard className="mb-4">
            <CCardBody>
              <CCardTitle className="mb-3">Kullanıcı Yorumları ({filteredReviews.length})</CCardTitle>
              <div className="mb-3">
              ...
              {filteredReviews && filteredReviews.length > 0 ? (
                ...
                <CAlert color="info">Henüz yorum yapılmamış</CAlert>
              )}
            </CCardBody>
          </CCard>
        </CCol>
        ...
        <CCol md={4}>
          {relatedProducts && relatedProducts.length > 0 && (
            <CCard className="mb-4">
              <CCardBody>
                <CCardTitle>Benzer Ürünler</CCardTitle>
                <CRow className="g-2">
